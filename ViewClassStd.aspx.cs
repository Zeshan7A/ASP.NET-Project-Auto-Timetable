...
public partial class ViewClassStd : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        //    con.Open();
        //    cmd.Connection = con;
        //    SqlDataAdapter sda = new SqlDataAdapter("select distinct Semester from ClassView where semester='" + Session["StdSemeter"].ToString() +"' group by Semester", con);
        ...
        if (!this.IsPostBack)
        {
            if (!string.IsNullOrEmpty(Session["Dept"] as string))
            {
                ...
                con.Open();
                cmd.Connection = con;
                SqlDataAdapter sda = new SqlDataAdapter("select distinct Semester from ClassView where semester='" + Session["StdSemeter"].ToString() + "' and Dept_Name='" + Session["Dept"].ToString() + "' group by Semester", con);
                DataTable tb = new DataTable();
                sda.Fill(tb);
                RepeaterSemester.DataSource = tb;
                RepeaterSemester.DataBind();
