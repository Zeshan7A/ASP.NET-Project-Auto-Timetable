...
using System.IO;
public partial class ViewTimetableFaculty : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!this.IsPostBack)
        {
            if (!string.IsNullOrEmpty(Session["Prof_Dept"] as string))
            {
                ...
                con.Open();
                cmd.Connection = con;
                SqlDataAdapter sda = new SqlDataAdapter("select distinct Semester from ClassView where  Dept_Name='" + Session["Prof_Dept"].ToString() + "' group by Semester", con);
    ...
    protected void RepeaterSemester_ItemDataBound1(object sender, RepeaterItemEventArgs e)
    {
        if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
        {
            ...
            SqlDataAdapter sda = new SqlDataAdapter("select Distinct Class_Code,Title,Name from ClassView  where Semester='" + sem + "' and Dept_Name='" + Session["Prof_Dept"].ToString() + "'", con);
